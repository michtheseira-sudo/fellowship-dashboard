...
import { toMonthlyPoints, pivotMonthlyByYear } from "@/lib/months";
import type { WeeklyPoint } from "@/lib/types";

// Colors assigned by recency rank, not hardcoded year numbers: most
// recent year = brand1 (primary/purple), next = brand2 (secondary/teal),
...
export default function MonthlyYoYChart({
  title,
  points,
  years,
}: {
  ...
  const monthly = toMonthlyPoints(points);
  const data = pivotMonthlyByYear(monthly);
